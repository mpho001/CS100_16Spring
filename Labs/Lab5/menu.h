...
class Menu {
	...
	public:
		...
		void execute();
			...
			//print 0 by default
		bool initialized(); 
			//Return true if the history has been primed with a single op
            //instruciton
			//This is necessary because that is the base of a calculation
		
		void add_command(Command* cmd); 
			//Adds a command to history in the appropriate posiiton
            //(based on history_index)
		
		Command* get_command();
			//Returns the command in history we are currently referring to
            //(based on history_index)
